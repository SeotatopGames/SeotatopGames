...
levelNumber = "1";
folderName = "/Level_" + levelNumber

print("----------Begin Level Export----------")

path = os.path.dirname(bpy.data.filepath)
file = open(os.path.join(path + "../../Texts/", "GameLevel_" + levelNumber + ".txt"),"w")
...
def print_heir(ob, levels=10):
    def recurse(ob, parent, depth):
        ...
        file.write(spaces + levelNumber + "_" +ob.name + "\n")
        ...
        if(ob.type == "LIGHT"):
            if(ob.data.type == "POINT"):
                ...
                converted[3][0:4] = ob.data.shadow_soft_size, radius, 0, 0
            if(ob.data.type == "SUN"):
...
export_dir = path + "../../Models" + folderName
...
for obj in bpy.context.scene.objects:
    ...
    if obj.type == 'MESH':
        ...
        if base_name not in unique_names:
            ...
            obj_file = os.path.join(export_dir, levelNumber + f"_{base_name}.obj")
...
if os.path.isfile(exe_path):
    ...
    for header in unique_names:
        os.remove(os.path.join(export_dir, levelNumber + f"_{header}.h"))
