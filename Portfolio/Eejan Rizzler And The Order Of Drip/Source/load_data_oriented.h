...
public:
	...
	{
		unsigned modelIndex, transformStart, transformCount, entityTypeIndex, flags; // flags optional
	};
	...
	struct BLENDER_OBJECT // *NEW* Used to track individual objects in blender
	{
		...
		unsigned int modelIndex, transformIndex, entityTypeIndex;
	};
	...
	std::vector<float> levelEntityTypes;
	...
	bool ReadGameLevel(const char* gameLevelPath,
		...
		while (+file.ReadLine(linebuffer, 1024, '\n'))
		{
			...
			if (std::strcmp(linebuffer, "MESH") == 0)
			{
				...
				file.ReadLine(linebuffer, 1024, '\n');
				std::sscanf(linebuffer, "%f", &add.entityType);
			...
			if (std::strcmp(linebuffer, "LIGHT") == 0)
			{
				file.ReadLine(linebuffer, 1024, '\n');
				std::string blenderName = linebuffer;
				log.LogCategorized("INFO", (std::string("Light Detected: ") + blenderName).c_str());
				// create the model file name from this (strip the .001)
				MODEL_ENTRY add = { linebuffer };
				add.modelFile = add.modelFile.substr(0, add.modelFile.find_last_of("."));
				add.modelFile += ".h2b";
				...
				for (int i = 0; i < 16; i++)
				{
					lightSettings.data[i] = transform.data[i];
				}

				levelLighting.push_back(lightSettings);
	...
	bool ReadAndCombineH2Bs(const char* h2bFolderPath,
		...
		for (auto i = modelSet.begin(); i != modelSet.end(); ++i)
		{
			if (p.Parse((modelPath + "/" + i->modelFile).c_str()))
			{
				...
				// add instance set
				levelEntityTypes.push_back(i->entityType);
				instances.entityTypeIndex = levelEntityTypes.size() - 1;
				...
				for (auto& n : i->blenderNames) {
					BLENDER_OBJECT obj{
						...
						instances.modelIndex, instances.transformStart + offset++, levelEntityTypes.size() - 1
					};
